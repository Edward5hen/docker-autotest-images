...
class rpms_signatures(sadc_base):
    ...
    def run_once(self):
        ...
        utils.run('sudo docker run -d --name=sadc_sig %s sleep 120' %
                  self.sub_stuff['img_name'])

        qa_cmd = 'sudo docker exec sadc_sig rpm -qa'
        query_cmd = 'sudo docker exec sadc_sig rpm -qi %s'
