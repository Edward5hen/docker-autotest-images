...
* support-tools container is installed
"""
...
class run_options(support_tools_base):
    ...
    def initialize(self):
        ...
        if 'support-tools' not in ctns.list_container_names():
            self.loginfo('Container support-tools is not found')
    ...
    def list_mounted_dir(self):
        dcr_exec_cmd = 'sudo docker exec support-tools '
    ...
    def echo_env_virs(self):
        dcr_exec_cmd = 'docker exec support-tools '
    ...
    def check_env_vir(self):
        ...
        # If testing by loading the image, NAME viriable is support-tools
        self.failif(
                not self.sub_stuff['env_vir_name'].startswith('support-tools'),
        ...
        # but definitely includes *support-tools*
        self.failif('support-tools' not in self.sub_stuff['env_vir_image'],
    ...
    def check_ipc(self):
        cmd_ctn = 'sudo docker exec support-tools ipcs -u'
        cmd_host = 'ipcs -u'
        self.sub_stuff['ipc_rst_ctn'] =\
    ...
    def check_net(self):
        cmd_ctn = 'sudo docker exec support-tools ifconfig'
        cmd_host = 'ifconfig'
        self.sub_stuff['net_rst_ctn'] =\
            self.format_output(utils.run(cmd_ctn).stdout)
        self.sub_stuff['net_rst_host'] =\
    ...
    def check_process(self):
        cmd_ctn = 'sudo docker exec support-tools ps -Ao pid,fname'
    ...
    def check_name(self):
        ...
        self.failif(result.strip() not in ['support-tools-docker', 'support-tools'],
                    'Container name is not support-tools-docker or support-tools')
    ...
    def postprocess(self):
        ...
        self.check_name()
        self.check_env_vir()
        # There is no ipcs command in container
        #self.check_ipc()
        # There is no ifconfig command in container
        #self.check_net()
        # There is no ps command in container
        #self.check_process()
        self.check_cves()
