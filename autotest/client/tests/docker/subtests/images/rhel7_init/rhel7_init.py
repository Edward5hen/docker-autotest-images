...
Operational Detail
----------------------
#. Check if rhel7-init has been loaded or not.
#. If not, load it.
...
*   Clean host without rhel7-init container ever installed
"""
import re
import fileinput
...
class rhel7_init(SubSubtestCaller):

    config_section = 'images/rhel7_init'


class rhel7_init_base(SubSubtest):

    def initialize(self):
        super(rhel7_init_base, self).initialize()
    ...
    def load_image(self):
        if not self.check_loaded():
            cmd = (
                ...
                'rhel7/rhel7-init:{}-{}'.format(self.config['ver'], self.config['rls_ver'])
                )
